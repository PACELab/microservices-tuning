...
class ComposePostHandler : public ComposePostServiceIf {
 ...
 private:
  ...
  /*
  // avs --begin
  queueContainer *composeQC = new queueContainer(ipPrintWindowInMS,composePost_initObjID,composePost_numQueueObjs);

  proxyQueues *q_UploadCreator =  composeQC->allQueueObjs[idx_ComposePost_UploadCreator]; 
  proxyQueues *q_UploadText =  composeQC->allQueueObjs[idx_ComposePost_UploadText]; 
  proxyQueues *q_UploadMedia =   composeQC->allQueueObjs[idx_ComposePost_UploadMedia];
  proxyQueues *q_UploadUniqueId =  composeQC->allQueueObjs[idx_ComposePost_UploadUniqueId];
  proxyQueues *q_UploadUrls =  composeQC->allQueueObjs[idx_ComposePost_UploadUrls]; 

  proxyQueues *q_UploadUserMentions = composeQC->allQueueObjs[idx_ComposePost_UploadUserMentions];
  proxyQueues *q_ComposeAndUpload = composeQC->allQueueObjs[idx_ComposePost_ComposeAndUpload]; 
  proxyQueues *q_UploadPostHelper = composeQC->allQueueObjs[idx_ComposePost_UploadPostHelper]; 
  proxyQueues *q_UploadUserTimelineHelper = composeQC->allQueueObjs[idx_ComposePost_UploadUserTimelineHelper]; 
  proxyQueues *q_UploadHomeTimelineHelper = composeQC->allQueueObjs[idx_ComposePost_UploadHomeTimelineHelper]; 
  // avs --end
 */
...
void ComposePostHandler::UploadUserMentions(
  ...
  //q_UploadUserMentions->incrReqs(); // avs
  // Initialize a span
  TextMapReader reader(carrier);
  std::map<std::string, std::string> writer_text_map;
  TextMapWriter writer(writer_text_map);
...
void ComposePostHandler::_ComposeAndUpload(
  ...
  //q_ComposeAndUpload->incrReqs(); // avs
  auto redis_client_wrapper = _redis_client_pool->Pop();
  if (!redis_client_wrapper) {
    ServiceException se;
    se.errorCode = ErrorCode::SE_REDIS_ERROR;
...
void ComposePostHandler::_UploadPostHelper(
  ...
  //q_UploadPostHelper->incrReqs(); // avs
  try{
...
void ComposePostHandler::_UploadUserTimelineHelper(
  ...
  //q_UploadUserTimelineHelper->incrReqs(); // avs
  try{
...
void ComposePostHandler::_UploadHomeTimelineHelper(
  ...
  //q_UploadHomeTimelineHelper->incrReqs(); // avs
  try {
