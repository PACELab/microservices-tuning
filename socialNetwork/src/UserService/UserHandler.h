...
//  queueContainer *userHandlerQC = new queueContainer(ipPrintWindowInMS,userHandler_initObjID,userHandler_numQueueObjs); // avs
//  proxyQueues *q_RegisterUserWithId =  userHandlerQC->allQueueObjs[idx_userHandler_RegisterUserWithId];   // avs
//  proxyQueues *q_RegisterUser =  userHandlerQC->allQueueObjs[idx_userHandler_RegisterUser];   // avs
//  proxyQueues *q_UploadCreatorWithUsername =  userHandlerQC->allQueueObjs[idx_userHandler_UploadCreatorWithUsername];   // avs

//  proxyQueues *q_UploadCreatorWithUserId =  userHandlerQC->allQueueObjs[idx_userHandler_UploadCreatorWithUserId];   // avs
//  proxyQueues *q_Login =  userHandlerQC->allQueueObjs[idx_userHandler_Login];   // avs
//  proxyQueues *q_GetUserId =  userHandlerQC->allQueueObjs[idx_userHandler_GetUserId];   // avs
...
void UserHandler::RegisterUserWithId(
  ...
  // q_RegisterUserWithId->incrReqs(); // avs
  // Initialize a span
...
//  q_UploadCreatorWithUsername->incrReqs(); // avs
  TextMapReader reader(carrier);
  std::map<std::string, std::string> writer_text_map;
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
...
//  q_UploadCreatorWithUserId->incrReqs(); // avs
  TextMapReader reader(carrier);
  std::map<std::string, std::string> writer_text_map;
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
