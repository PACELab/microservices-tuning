...
//  queueContainer *postStorageQC = new queueContainer(ipPrintWindowInMS,postStorage_initObjID,postStorage_numQueueObjs); // avs
//  proxyQueues *q_storePost =  postStorageQC->allQueueObjs[idx_postStorage_storePost];   // avs
//  proxyQueues *q_readPost =  postStorageQC->allQueueObjs[idx_postStorage_readPost];   // avs
//  proxyQueues *q_readMultiplePosts =  postStorageQC->allQueueObjs[idx_postStorage_readMultiplePosts];   // avs
...
//  q_readMultiplePosts->incrReqs(); //avs
  // Initialize a span
  TextMapReader reader(carrier);
  std::map<std::string, std::string> writer_text_map;
  TextMapWriter writer(writer_text_map);
