...
//  queueContainer *userTimelineQC = new queueContainer(ipPrintWindowInMS,userHandler_initObjID,userHandler_numQueueObjs); // avs
//  proxyQueues *q_ReadUserTimeline =  userTimelineQC->allQueueObjs[idx_userTline_ReadUserTimeline];   // avs
//  proxyQueues *q_WriteUserTimeline =  userTimelineQC->allQueueObjs[idx_userTline_WriteUserTimeline];   // avs
...
//  q_WriteUserTimeline->incrReqs(); //avs
  // Initialize a span
...
void UserTimelineHandler::ReadUserTimeline(
  ...
  //q_ReadUserTimeline->incrReqs(); //avs
  // Initialize a span
  TextMapReader reader(carrier);
  std::map<std::string, std::string> writer_text_map;
  TextMapWriter writer(writer_text_map);
