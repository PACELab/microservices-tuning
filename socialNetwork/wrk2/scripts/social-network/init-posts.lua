...
local global_user_index = 0
function init(args) 
     for i, v in ipairs(args) do
         -- print(i,v)
         global_user_index=tonumber(v)
     end 
 end 
...
request = function(args)
  ...
  local user_index = 0
  if global_user_index > 962 then
    user_index = math.random(1,962)
  else
    user_index = global_user_index
  end
  ...
  local path = "http://userv-large-1:8080/wrk2-api/post/compose"
  local headers = {}
  local body
